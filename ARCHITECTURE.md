...
**Enlaces entre notas (`[[Título de la nota]]`) resueltos en el cliente, no en el backend.** El formulario de nota extrae la sintaxis `[[...]]` del contenido (`features/notas/links.ts`) y la resuelve contra la lista de notas ya cargada en el cliente — sin tabla de relación propia ni endpoint nuevo: el contenido completo de todas las notas ya viaja al frontend para poder editarlas, así que recorrerlo basta tanto para saber a qué notas enlaza la que se está editando como, a la inversa, qué notas la mencionan a ella (backlinks). Comparación por título sin distinguir mayúsculas/minúsculas; un enlace a un título que no existe se ignora sin más (sin creación implícita de notas ni seguimiento de enlaces rotos, para no complicar lo que sigue siendo notas de texto plano). Al implementarlo, la pantalla de notas pasó a pedir la lista dos veces (una filtrada por la etiqueta activa para la vista, otra sin filtrar para resolver enlaces contra notas fuera de ese filtro) con dos instancias independientes de `useNotes` — antes de publicarse se simplificó a una sola fuente sin filtrar, aplicando el filtro por etiqueta en el cliente sobre esa misma lista: evita la petición duplicada y evita que la lista usada para resolver enlaces quedara desactualizada tras crear/editar/borrar una nota con un filtro activo.
